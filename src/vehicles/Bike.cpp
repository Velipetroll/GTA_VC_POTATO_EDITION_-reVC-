...
{
	// --- INICIO HACK 1RA PERSONA: Restaurar físicas ---
	if (m_bike_unused1 != 20.0f) {
		m_fLeanInput = m_bike_unused1;
	}
	// --- FIN HACK ---
	...
	if (skipPhysics) {
		...
		// --- INICIO MEJORA VELOCIDAD ---
		float savedAirResistance = m_fAirResistance;
		if (GetStatus() == STATUS_PLAYER && pDriver) {
			// Aplicamos impulso constante e ignoramos la animación
			if (m_fLeanInput > 0.15f) {
				m_fAirResistance *= 0.6f;
				if (m_fGasPedal > 0.5f && DotProduct(m_vecMoveSpeed, GetForward()) > 0.25f) {
					ApplyMoveForce(0.035f * m_fMass * GRAVITY * CTimer::GetTimeStep() * GetForward());
					bExtraSpeed = true;
				}
			}
		}

		CPhysical::ProcessControl(); // ¡NO BORRAR ESTA LÍNEA O SE ROMPE EL JUEGO!
	...
	if (m_fHealth < 250.0f && GetStatus() != STATUS_WRECKED) {
		...
		CParticle::AddParticle(PARTICLE_CARFLAME, damagePos, fireDir, nil, 0.9f);
	...
	// --- INICIO HACK 1RA PERSONA: Engañar a Tommy ---
	m_bike_unused1 = m_fLeanInput;
